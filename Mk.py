...
import telebot, requests, os, sys, time, logging, threading, csv, io, queue
...
DB_FILE        = "database.db"
TURSO_DATABASE_URL = "libsql://mkfullcompletebot-muzahidkagushi-lab.aws-ap-south-1.turso.io"
TURSO_AUTH_TOKEN    = "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3ODMwNDM4MTAsImlkIjoiMDE5ZjI1YWYtMDgwMS03NzU3LWJjNDctZjE0MThhZTc3YjRkIiwia2lkIjoick9SNGw2SlVNaGhYZlZsWU1XeC1IQWFaREt1OVlrbzFUT0x2MFFrcERDSSIsInJpZCI6ImM5MzliMzhkLWM1ZTMtNDczOS1hZDcxLTZmOTFhZDFhNTNmMCJ9.KSxvg627BnS54nh3syIv9eofZ48XFGIXAmfHYMT52WWORRsjkzHQXBkcmewO6KAWqnV1muct7cDnQJ3_D8KUAQ"
TURSO_HTTP_URL      = TURSO_DATABASE_URL.replace("libsql://", "https://").rstrip("/")
LOG_FILE       = "error.log"
...
# ════════════════════════════════════════════════════════════
# DATABASE (Turso / libSQL — via HTTP API, no native package needed)
# ════════════════════════════════════════════════════════════
import base64 as _b64

# Persistent HTTP session — reuses TCP/TLS connections instead of opening a
# fresh one for every single query (this alone removes a huge chunk of
# per-request latency over mobile networks).
_session = requests.Session()

# ────────────────────────────────────────────────────────────
# SPEED LAYER: in-memory cache + write-behind queue
#
# Idea: reads never touch the network (served from RAM), writes are
# applied to RAM immediately (so the bot's reply always reflects the
# change instantly) and pushed to Turso in the background by a single
# worker thread. This means a command like /start or a balance check
# responds in milliseconds instead of waiting on a Mumbai round-trip.
#
# Trade-off (be aware): if the process is killed before a queued write
# reaches Turso, that last write is lost. For a Termux bot this is a
# reasonable trade for instant responses; if you need zero write loss,
# this isn't the pattern for that.
# ────────────────────────────────────────────────────────────
_cache_lock  = threading.Lock()
USER_CACHE   = {}          # uid -> dict of the users row
_write_queue = queue.Queue()

def _enqueue_write(sql, params):
    _write_queue.put((sql, params))

def _cache_set(uid, **fields):
    """Keep USER_CACHE consistent whenever code updates the users table
    directly (admin actions, check-in, etc.) instead of through reg()/
    add_bal()/deduct_bal(). No-op if the user isn't cached yet — the next
    get_user() call will just fetch the fresh row."""
    with _cache_lock:
        u=USER_CACHE.get(uid)
        if u is not None: u.update(fields)

def _db_writer_loop():
    conn = None
    while True:
        sql, params = _write_queue.get()
        for attempt in range(3):
            try:
                ...
                logger.error(f"async write failed (attempt {attempt+1}): {e} | sql={sql[:60]}")
                conn = None
                time.sleep(1)
...
def _to_arg(v):
    ...
    if isinstance(v, (bytes, bytearray)):
        return {"type": "blob", "base64": _b64.b64encode(v).decode()}
...
class _Cursor:
    ...
    def execute(self, sql, params=()):
        ...
        self._cols = res["cols"]; self._rows = res["rows"]; self._idx = 0
        self.lastrowid = res["last_insert_rowid"]
        self.rowcount = res["affected_row_count"]
        return self
    ...
    def fetchall(self):
        rows = [_Row(self._cols, r) for r in self._rows[self._idx:]]
...
class _Conn:
    ...
    def _run(self, sql, params):
        payload_req = {"type": "execute", "stmt": {"sql": sql, "args": [_to_arg(p) for p in (params or ())]}}
        payload = {"requests": [payload_req]}
        if self._baton:
            payload["baton"] = self._baton
        last_err = None
        for attempt in range(4):
            try:
                r = _session.post(f"{self._base}/v2/pipeline", headers=self._headers,
        ...
        if not results:
            return {"cols": [], "rows": [], "last_insert_rowid": None, "affected_row_count": -1}
        item = results[0]
        if item.get("type") == "error":
            raise Exception(item.get("error"))
        res = item["response"]["result"]
        cols = [c.get("name") for c in res.get("cols", [])]
        rows = [[_from_cell(cell) for cell in row] for row in res.get("rows", [])]
        lrid = res.get("last_insert_rowid")
        return {
            "cols": cols,
            "rows": rows,
            "last_insert_rowid": int(lrid) if lrid is not None else None,
            "affected_row_count": res.get("affected_row_count", -1),
        }
    def cursor(self):
        return _Cursor(self)
    def execute(self, sql, params=()):
    ...
    def commit(self):
        pass
    def close(self):
        # Skipping the network "close" call on purpose: each get_conn() is used
        # once per handler and Turso auto-expires idle batons on its own, so
        # sending an extra HTTP request here just doubles our round-trips for
        # no benefit. This alone removes ~half of the network calls per command.
        self._baton = None

def get_conn():
    return _Conn(TURSO_HTTP_URL, TURSO_AUTH_TOKEN)
...
def init_db():
    ...
    defs = {
        ...
        "vpn_store_link":"http://t.me/vpn_store_primumea_apps_bot",
        "vpn_store_text":"🛡️ VPN 🛍️ Shops\n\nফ্রি এবং অল্প কিছু টাকার বিনিময়ে প্রিমিয়াম ভিপিএন ও প্রিমিয়াম অ্যাপস পেতে নিচের বাটনে ট্যাপ করুন।",
    }
    for k,v in defs.items():
        c.execute("INSERT OR IGNORE INTO bot_settings VALUES (?,?)",(k,v))
    for pk,pn,pr,pd in [("basic","Basic Plan",20,1),("pro","Pro Plan",100,30),("ultra","Ultra Plan",1080,365)]:
        c.execute("INSERT OR IGNORE INTO hosting_plans VALUES (?,?,?,?,1)",(pk,pn,pr,pd))
...
def _load_settings_cache():
    """Blocking refresh — only ever called at startup and from the
    background refresh thread, never from a message handler."""
    conn=get_conn()
    try:
        rows=conn.execute("SELECT key,value FROM bot_settings").fetchall()
        with _cache_lock:
            _gs_cache.clear()
            _gs_cache.update({r["key"]:r["value"] for r in rows})
    finally: conn.close()

def _settings_refresh_loop():
    while True:
        time.sleep(20)
        try: _load_settings_cache()
        except Exception as e: logger.error(f"settings refresh failed: {e}")

def gs(k,fb=""): # get setting — pure in-memory read, never blocks on network
    return _gs_cache.get(k,fb)

def ss(k,v): # set setting — instant locally, persisted in the background
    _gs_cache[k]=str(v)
    _enqueue_write("INSERT OR REPLACE INTO bot_settings VALUES (?,?)",(k,str(v)))
...
def reg(user):
    uid=user.id; now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with _cache_lock: cached=USER_CACHE.get(uid)
    if cached is not None:
        # returning user — update RAM instantly, persist in the background
        cached["last_active"]=now; cached["name"]=user.first_name or ""; cached["username"]=user.username or ""
        _enqueue_write("UPDATE users SET last_active=?,name=?,username=? WHERE user_id=?",
                       (now,user.first_name or "",user.username or "",uid))
        return
    # brand-new user (never seen this process) — one blocking round trip to
    # create the row, then it's cached for every future command
    conn=get_conn()
    try:
        conn.execute("INSERT OR IGNORE INTO users (user_id,name,username,joined_at,last_active) VALUES (?,?,?,?,?)",
                     (uid,user.first_name or "",user.username or "",now,now))
    ...
    if row:
        with _cache_lock: USER_CACHE[uid]=dict(zip(row.keys(),list(row)))

def get_user(uid):
    with _cache_lock: cached=USER_CACHE.get(uid)
    if cached is not None: return cached
    conn=get_conn()
    try: row=conn.execute("SELECT * FROM users WHERE user_id=?",(uid,)).fetchone()
    finally: conn.close()
    if not row: return None
    d=dict(zip(row.keys(),list(row)))
    with _cache_lock: USER_CACHE[uid]=d
    return d

def get_balance(uid):
    r=get_user(uid); return float(r["balance"]) if r else 0.0

def get_lang(uid):
    r=get_user(uid); return r["language"] if r else "bn"

def add_bal(uid,amt,note=""):
    now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    u=get_user(uid)
    if u:
        with _cache_lock: u["balance"]=float(u.get("balance") or 0)+amt
    _enqueue_write("UPDATE users SET balance=balance+? WHERE user_id=?",(amt,uid))
    _enqueue_write("INSERT INTO transaction_logs (user_id,tx_type,amount,note,created_at) VALUES (?,?,?,?,?)",
                   (uid,"credit",amt,note,now))

def deduct_bal(uid,amt,note=""):
    now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    u=get_user(uid)
    if not u or float(u.get("balance") or 0)<amt: return False
    with _cache_lock: u["balance"]=float(u["balance"])-amt
    _enqueue_write("UPDATE users SET balance=balance-? WHERE user_id=?",(amt,uid))
    _enqueue_write("INSERT INTO transaction_logs (user_id,tx_type,amount,note,created_at) VALUES (?,?,?,?,?)",
...
def mark_pass_verified(uid):
    tok=pass_token()
    _cache_set(uid,pass_verify=tok)
    _enqueue_write("UPDATE users SET pass_verify=? WHERE user_id=?",(tok,uid))
...
def cmd_start(msg):
    ...
    u=get_user(uid)  # fetch once, reuse below instead of re-querying per check
    lang=(u["language"] if u else "bn")
    if u and u["is_banned"]: bot.send_message(uid,"🚫 আপনাকে ব্যান করা হয়েছে।"); return
    admin_now = is_super(uid) or (u and u["role"] in ("admin","moderator") and (u["pass_verify"] or "")==pass_token())
    if is_maint() and not admin_now: bot.send_message(uid,"🔧 বট রক্ষণাবেক্ষণে আছে।"); return

    args=msg.text.split()
    if len(args)>1 and args[1].startswith("ref_"):
        try:
            ref_uid=int(args[1][4:])
            if ref_uid!=uid and u and u.get("referred_by") is None:
                ...
                _cache_set(uid,referred_by=ref_uid)
                r_ref=get_user(ref_uid)
                if r_ref:
                    _cache_set(ref_uid,balance=float(r_ref.get("balance") or 0)+bonus,
                               referral_count=int(r_ref.get("referral_count") or 0)+1,
                               total_earned=float(r_ref.get("total_earned") or 0)+bonus)
    ...
    prefix="⭐ প্রিমিয়াম — " if (u and u["is_premium"]) else ""
    suffix="\n\n📋 নিচের মেনু থেকে সেবা বেছে নিন:" if lang=="bn" else "\n\n📋 Select a service:"
    kb=get_admin_kb(uid) if admin_now else main_kb(lang)
...
def handle_text(msg):
    try:
        ...
        if state.startswith("await_wd_number_"):
            ...
            try:
                ...
                if row2 and row2["last_wd_date"] and str(row2["last_wd_date"]).startswith(today):
                    new_count=int(row2["withdraw_today"] or 0)+1
            ...
            _cache_set(uid,withdraw_today=new_count,last_wd_date=today)
        ...
        if state.startswith("adm_setrole_role_"):
            ...
            _cache_set(target_id,role=role)
        ...
        if state.startswith("adm_editlimit_"):
            ...
            smap={"mindep":"min_deposit","minwd":"min_withdraw","wdcharge":"withdraw_charge","maxwd":"max_withdraw_day",
                  ...
                  "vpnlink":"vpn_store_link","vpntext":"vpn_store_text"}
        ...
        if state=="adm_premium_id":
            ...
            try:
                ...
                _cache_set(target,is_premium=new_val)
        ...
        if text=="VPN🛡️ Shops 🛍️":
            ikb=types.InlineKeyboardMarkup()
            ikb.add(types.InlineKeyboardButton("🛍️ Go To Store",url=gs("vpn_store_link","http://t.me/vpn_store_primumea_apps_bot")))
            bot.send_message(uid,gs("vpn_store_text","🛡️ VPN 🛍️ Shops"),reply_markup=ikb); return
        ...
        if text=="🇧🇩 বাংলা":
            _cache_set(uid,language="bn")
            _enqueue_write("UPDATE users SET language='bn' WHERE user_id=?",(uid,))
            bot.send_message(uid,"✅ ভাষা সেট হয়েছে।",reply_markup=main_kb("bn")); return
        if text=="🇬🇧 English":
            _cache_set(uid,language="en")
            _enqueue_write("UPDATE users SET language='en' WHERE user_id=?",(uid,))
        ...
        if text=="🛡️ VPN Store সেটিংস":
            kb_vpn=types.InlineKeyboardMarkup()
            kb_vpn.add(types.InlineKeyboardButton(f"🔗 লিংক: {gs('vpn_store_link')}",callback_data="editlimit_vpnlink"))
            kb_vpn.add(types.InlineKeyboardButton("✏️ বার্তা এডিট করুন",callback_data="editlimit_vpntext"))
            kb_vpn.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel"))
            bot.send_message(uid,f"🛡️ VPN Store সেটিংস:\n\n📝 বর্তমান বার্তা:\n{gs('vpn_store_text')}",reply_markup=kb_vpn); return
...
def handle_callback(call):
    try:
        ...
        if data.startswith("wd_cancel_") and is_admin(uid) and has_perm(uid,"withdrawals"):
            ...
            r_t=get_user(tuid)
            if r_t: _cache_set(tuid,withdraw_today=max(0,int(r_t.get("withdraw_today") or 0)-1))
        ...
        limit_map={"editlimit_mindep":"💳 নতুন সর্বনিম্ন ডিপোজিট (BDT):","editlimit_minwd":"💸 নতুন সর্বনিম্ন উইথড্র (BDT):",
                   ...
                   "editlimit_vpnlink":"🔗 নতুন VPN Store লিংক দিন (যেমন http://t.me/yourbot):",
                   "editlimit_vpntext":"✏️ ইনলাইন কীবোর্ডের নতুন বার্তা লিখুন (একাধিক লাইন লেখা যাবে):"}
...
def _do_ban(admin_uid,text,ban):
    try:
        ...
        _cache_set(target,is_banned=1 if ban else 0)
...
def _do_checkin(uid,lang):
    ...
    bonus=float(gs("daily_checkin_bonus",str(DAILY_CHECKIN))); now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    _cache_set(uid,last_checkin=now,balance=float(row.get("balance") or 0)+bonus)
    _enqueue_write("UPDATE users SET last_checkin=?,balance=balance+? WHERE user_id=?",(now,bonus,uid))
    _enqueue_write("INSERT INTO transaction_logs (user_id,tx_type,amount,note,created_at) VALUES (?,?,?,?,?)",
                   (uid,"credit",bonus,"daily checkin",now))
    bal=get_balance(uid)
...
if __name__=="__main__":
    ...
    print("="*55)
    init_db(); migrate_db()
    _load_settings_cache()  # one blocking call at startup so gs() never blocks a handler
    threading.Thread(target=_db_writer_loop,daemon=True).start()
    threading.Thread(target=_settings_refresh_loop,daemon=True).start()
