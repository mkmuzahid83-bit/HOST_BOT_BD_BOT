...
import telebot, requests, os, sys, time, logging, threading, csv, io
...
class _Conn:
    ...
    def _run(self, sql, params):
        ...
        for attempt in range(4):
            try:
                r = requests.post(f"{self._base}/v2/pipeline", headers=self._headers,
    ...
    def close(self):
        if self._baton:
            try:
                requests.post(f"{self._base}/v2/pipeline", headers=self._headers,
                               json={"baton": self._baton, "requests": [{"type": "close"}]}, timeout=10)
            except Exception:
                pass
            self._baton = None
